...
tab1, tab2 = st.tabs(["🛡️ Threat Intel (Live)", "📈 Histórico"])
        ...
        with st.spinner("Interceptando feeds globais de ameaças..."):
            try:
                ...
                if not news_data:
                    ...
                    # 7. Ações Finais Ocultas/Download (Salvar DB & Exportar PDF)
...
with tab2:
    ...
    if not historical_df.empty:
        fig_hist = px.bar(
            ...
            color="Sector",
            text_auto=True,
            title="Setores Globais Atacados no Histórico"
        )
        fig_hist.update_layout(showlegend=False)
        st.plotly_chart(fig_hist, use_container_width=True)
    else:
        st.info("Seu banco de dados histórico está vazio. Volte para a aba '# Threat Intel (Live)' e gere um relatório!")
